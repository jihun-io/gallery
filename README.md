...
초기에는 Cloudflare R2를 사용했으나, 데이터베이스와 애플리케이션은 물리 서버에 위치하고 이미지만 외부 CDN에 존재하여 레이턴시가 발생했습니다. 이를 해결하기 위해 스토리지를 서버 내 컨테이너로 구축하기로 결정했고, S3 호환 API를 제공하는 MinIO를 선택하여 기존 코드 수정을 최소화했습니다.
